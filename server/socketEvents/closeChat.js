...
const closeChat = (activeDialogues, standartParameters) => {
    ...
    activeDialogues.forEach((value, index) => {
        ...
        if (value.firstUserId === senderSessionId || value.secondUserId === senderSessionId) {
            firstUserForReject = first;
            secondUserForReject = second;

            activeDialogues.splice(index);
            
            io.to(findIdBySessionId(firstUserForReject, connectedUsersList)).emit('rejectChat', 'User close chat');
            io.to(findIdBySessionId(secondUserForReject, connectedUsersList)).emit('rejectChat', 'User close chat');
