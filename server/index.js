...
try {
    io.on('connection', socket => {
        ...
        socket.on('setUsername', data =>
            setUserNickname(data, connectedUsersList, socket.id),
        );

        socket.on('findDialogueById', searchedId =>
            findDialogueById(searchedId, waitConfirmationUsers, standartParameters),
        );

        socket.on('acceptChat', data =>
            acceptChat(data, activeDialogues, waitConfirmationUsers, standartParameters),
        );

        socket.on('discardChat', data =>
            discardChat(data, waitConfirmationUsers, standartParameters),
        );

        socket.on('closeChat', data => closeChat(activeDialogues, standartParameters));

        socket.on('sendMessage', data =>
            redirectMessage(data, activeDialogues, standartParameters),
        );

        socket.on('getUsers', () => {
            const convertedMap = [...connectedUsersList].map(([name, value]) => ({
                name,
                value,
            }));
        ...
        socket.on('disconnect', data => {
            rejectChat(
                socket.id,
                'Your companion was disconnected',
                activeDialogues,
                standartParameters,
            );
