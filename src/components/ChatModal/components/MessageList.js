...
function MessageList({ messages }) {
    ...
    function renderMessages() {
        ...
        for (let i = 0; i < messages.length; i++) {
            ...
            const finalClassname = item?.isClientMessage
                ? defaultClassname + 'rightSideMessage'
                : defaultClassname + 'leftSideMessage';

            messageList.push(
                <div className={finalClassname} key={i}>
                    <MessageCard
    ...
    return <div className="chatModal_MessageList">{renderMessages()}</div>;
}
